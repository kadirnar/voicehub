...
_CHECKPOINT_STATUS_OVERRIDES = {
    ...
    "vad_webrtc":
    "Weightless algorithm; version the implementation and configuration, not model weights",
    "vad_auditok":
    "Weightless energy detector; version the implementation and configuration, not model weights",
}
...
def checkpoint_documentation(spec) -> CheckpointDocumentation:
    ...
    hugging_face_url = (None if hugging_face_id is None else f"https://huggingface.co/{hugging_face_id}")
...
INFERENCE_PROFILES = {
    ...
    _asr(
        ...
        arguments=(
            'language="en"', 'prompt="Technical meeting with VoiceHub terminology"', "do_sample=False"),
    ),
    "asr_vibevoice":
    _asr(
