...
| Base classes | `/voicehub/models/` | Top level, second; model lifecycle and support, contribution, preprocessors, and architecture | Present |
| Inference | `/voicehub/guides/inference/` | Top level, third; task pipelines and serving | Present |
| Training | `/voicehub/guides/trainer/` | Top level, fourth; Trainer overview and fine-tuning | Present |
...
The rendered English home exposed all eight labels in the exact configured
order in VoiceHub's light and dark themes. At 1440 x 900 they fit in the
1,043-pixel tab strip without overflow. At 1024 x 768 Material moved the same
hierarchy into the persistent 218-pixel navigation region. At 390 x 844 the
opened 242-pixel drawer exposed all eight root sections, including the wrapped
optimization label, without page overflow. Keyboard focus on the first desktop
tab and the tablet and mobile API links rendered a two-pixel indigo outline
with a two-pixel offset. The upstream rendered navigation and its versioned
toctree exposed the corresponding eight groups in the same order.
...
theme/source gap. All eight root sections are visible in the rail. At desktop
...
| Installation | `/docs/transformers/main/en/installation` | `/voicehub/getting-started/installation/` | The current Virtual environment, Python, Source install, Editable install, conda, Set up, Cache directory, and Offline mode hierarchy, active navigation, exact right TOC, and absence of stale content tabs are mapped | Current local contract: all six viewport/palette cases pass the exact nine-heading and eight-entry TOC order, zero stale tab sets, 15 code blocks and code-copy actions, one page-copy action, four external and three internal content destinations, edit and previous/next targets, installation and offline-safety markers, geometry, Axe, reviewed screenshot signatures, and complete focus traversal. Six additional cases activate the first code copy and six activate page copy by keyboard, verify exact clipboard text plus visible success and idle states, retain focus and zero overflow, and rerun Axe |
