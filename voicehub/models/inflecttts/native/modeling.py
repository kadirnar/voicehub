"""Native PyTorch implementation of the Inflect Micro/Nano v2 graph."""
...
from voicehub.models.inflecttts.native import attentions, commons, modules
from voicehub.models.inflecttts.native.alias_free import AliasFreeActivation1d, AliasFreeResBlock1, SnakeBeta
from voicehub.models.inflecttts.native.commons import get_padding, init_weights
...
class Generator(torch.nn.Module):
    ...
    def __init__(
            ...
            gin_channels=0,
            decoder_alias_free=False,
            decoder_alias_free_start_stage=2,
            decoder_snake_logscale=True):
        super().__init__()
        ...
        for i in range(len(self.ups)):
            ...
            for j, (k, d) in enumerate(zip(resblock_kernel_sizes, resblock_dilation_sizes)):
                if self.decoder_alias_free and i >= self.decoder_alias_free_start_stage:
                    self.resblocks.append(AliasFreeResBlock1(ch, k, d, logscale=decoder_snake_logscale))
                else:
...
def build_inflect_model(config: InflectV2Config) -> SynthesizerTrn:
    """Construct the exact graph declared by one validated release config."""
