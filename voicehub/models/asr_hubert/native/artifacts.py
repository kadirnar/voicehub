"""Coherent local and Hugging Face artifact resolution for HuBERT CTC."""
...
class HubertArtifacts:
    """Immutable files forming one native HuBERT CTC runtime."""
...
def _required_local(root: Path, filename: str) -> Path:
    path = root / filename
    if not path.is_file():
        raise FileNotFoundError(f"Native HuBERT requires {filename!r} in {root}.")
    return path
...
def _safe_shard_names(index_path: Path) -> tuple[str, ...]:
    ...
    if not isinstance(weight_map, dict) or not weight_map:
        raise ValueError("HuBERT Safetensors index must contain a non-empty `weight_map`.")
    names: set[str] = set()
    for tensor_name, shard_name in weight_map.items():
        if not isinstance(tensor_name, str) or not tensor_name:
            raise ValueError("HuBERT Safetensors index contains an invalid tensor name.")
        if not isinstance(shard_name, str) or not shard_name:
            raise ValueError("HuBERT Safetensors index contains an invalid shard name.")
        shard_path = PurePosixPath(shard_name)
        if ("\\" in shard_name or shard_path.is_absolute() or len(shard_path.parts) != 1 or
                ".." in shard_path.parts):
            raise ValueError(f"Unsafe HuBERT checkpoint shard path {shard_name!r}.")
        if not shard_name.endswith(".safetensors"):
            raise ValueError(f"HuBERT checkpoint shard is not Safetensors: "
                             f"{shard_name!r}.")
...
def _validate_checkpoint(path: Path) -> None:
    if (path.suffix != ".safetensors" and not path.name.endswith(".safetensors.index.json")):
        raise ValueError("Native HuBERT accepts a Safetensors checkpoint or index.")
...
def resolve_hubert_artifacts(
    ...
    *,
    ...
    if not isinstance(source, (str, Path)) or not str(source).strip():
        raise ValueError("HuBERT `source` must be a non-empty path or Hub ID.")
    _validate_filename(
        checkpoint_filename,
    ...
    if is_explicit_local_path(source):
        raise FileNotFoundError(f"HuBERT model path was not found: {source_path}.")
    ...
    if checkpoint is None:
        ...
        raise FileNotFoundError(
            "Native HuBERT could not find a Safetensors checkpoint among: "
            f"{names}. Legacy `.bin` checkpoints are intentionally not "
            "deserialized.")
    if checkpoint.name.endswith(".safetensors.index.json"):
        for shard in _safe_shard_names(checkpoint):
...
__all__ = ["HubertArtifacts", "resolve_hubert_artifacts"]
