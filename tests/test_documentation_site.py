...
class DocumentationSiteTests(unittest.TestCase):
    ...
    def test_model_index_lists_every_registered_model(self):
        ...
        headings = (
            "# Model list",
            *(f"## {task_labels[task]}" for task in task_order),
            "## Registry access",
        )
        ...
        self.assertEqual(index.count('<div class="vh-model-catalog" markdown>'), len(task_order))
        self.assertEqual(
            index.count("| Model | Languages | Hugging Face ID | Training | Notebook |"),
            len(task_order),
        )
        ...
        self.assertEqual(len(catalog_links), len(specs))
        self.assertEqual(len({path for _name, path in catalog_links}), len(specs))
        for task_index, task in enumerate(task_order):
            section = index.split(f"## {task_labels[task]}", 1)[1]
            if task_index + 1 < len(task_order):
                section = section.split(f"## {task_labels[task_order[task_index + 1]]}", 1)[0]
            else:
                section = section.split("## Registry access", 1)[0]
            expected_links = tuple((spec.display_name, f"{spec.model_type}.md") for spec in sorted(
                (candidate for candidate in specs if candidate.task.value == task),
                key=lambda candidate: (candidate.display_name.casefold(), candidate.model_type),
            ))
            self.assertEqual(
                tuple(re.findall(r"^\| \[`([^`]+)`\]\(([^)]+\.md)\) \|", section, re.MULTILINE)),
                expected_links,
            )
        self.assertIn("- Model list: models/providers/index.md", config)
        self.assertNotIn("javascripts/model-explorer.js", config)
        self.assertFalse((DOCS_ROOT / "javascripts" / "model-explorer.js").exists())
        ...
        for spec in specs:
            with self.subTest(model_type=spec.model_type):
                ...
                checkpoint = model_documentation["checkpoint_documentation"](spec)
                if checkpoint.hugging_face_id is None:
                    self.assertIn("| Not published / not applicable |", row)
                else:
                    self.assertIn(
                        f"[`{checkpoint.hugging_face_id}`]({checkpoint.hugging_face_url})",
                        row,
                    )
                self.assertIn(f"| `{spec.training.support.value}` |", row)
                colab = (
                    "https://colab.research.google.com/github/kadirnar/voicehub/blob/main/"
                    f"notebooks/models/{spec.model_type}.ipynb")
                self.assertEqual(colab in row, checkpoint.is_hugging_face)
        ...
        for fragment in (
                ...
                "MODEL_INDEX_TOC",
                "MODEL_INDEX_MINIMUM_INTERSECTING_ROWS",
        ...
        for fragment in (
            ...
            self.assertNotIn(fragment, checker)
...
"""
    ...
    def test_external_archive_checkpoint_records_the_verified_mirror(self):
        ...
        index_row = next(
            line for line in index.splitlines() if line.startswith("| [`WeNetASR`](asr_wenet.md) |"))
        self.assertIn("| Not published / not applicable |", index_row)
        self.assertNotIn("huggingface.co", index_row)
        self.assertNotIn("[Colab]", index_row)
            ...
            with self.subTest(model_type=spec.model_type):
                ...
                if spec.task is SpeechTask.VOICE_ACTIVITY_DETECTION:
                    ...
                    row = next(
                        line for line in index.splitlines()
                        if line.startswith(f"| [`{spec.display_name}`]({spec.model_type}.md) |"))
                    self.assertIn("| Not text-language conditioned |", row)
                else:
                    ...
                    index_languages = (
                        ", ".join(f"`{code}`" for code in support.codes[:4]) +
                        ", … complete audited list on the model page"
                        if len(support.codes) > 8 else rendered_codes)
                    row = next(
                        line for line in index.splitlines()
                        if line.startswith(f"| [`{spec.display_name}`]({spec.model_type}.md) |"))
                    self.assertIn(f"| {index_languages} |", row)
